...
export default function NursesManagementPage() {
  ...
  const fetchNurses = async () => {
    try {
      ...
      let nursesList = Array.isArray(userData) ? userData : [];

      if (userRole === "clinic_owner" && userClinicId) {
        nursesList = nursesList.filter(
          (nurse: any) => nurse.clinic_id === userClinicId
        );
      }
      ...
      toast({
        title: "❌ Fehler",
        description:
          error.message || "Krankenpfleger konnten nicht geladen werden",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleToggleStatus = async (id: number, currentStatus: string) => {
    try {
      ...
      toast({
        title: "✅ Erfolg",
        description: "Krankenpflegerstatus erfolgreich aktualisiert",
        variant: "success",
      });
      fetchNurses();
    } catch (error: any) {
    ...
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            Krankenpflegerverwaltung
          </h1>
          <p className="text-gray-600 mt-1">
            Verwalten Sie alle Krankenpfleger im System
          </p>
        </div>
        <Link href="/admin/users/add">
          <Button className="bg-green-600 hover:bg-green-700">
            <Plus className="h-4 w-4 mr-2" />
            Krankenpfleger hinzufügen
          </Button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Aktive</p>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Inaktive</p>
      ...
      <Card className="p-4">
        <div className="flex items-center space-x-2">
          ...
          <Input
            placeholder="Krankenpfleger suchen..."
            value={searchTerm}
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="text-center py-12">
              <p className="text-gray-500">Lädt Krankenpfleger...</p>
            </div>
          ) : nurses.length === 0 ? (
            <div className="text-center py-12">
              <Heart className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500">Keine Krankenpfleger gefunden</p>
            </div>
          ) : (
                ...
                {nurses.map((nurse) => (
                  <tr key={nurse.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        variant={
                          nurse.status === "active" ? "default" : "secondary"
                        }
                        className={
                          nurse.status === "active"
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleToggleStatus(nurse.id, nurse.status)
                          }
                        >
                          <Power className="h-4 w-4" />
                        </Button>
                        <Link href={`/admin/users/edit/${nurse.id}`}>
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleDeleteClick(nurse.id, nurse.name)
                          }
